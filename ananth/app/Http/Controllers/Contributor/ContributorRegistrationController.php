...
class ContributorRegistrationController extends Controller
{
    ...
    public function submit(Request $request)
    {
        $request->validate([
            ...
            'plan'               => 'required|in:paid_standard,paid_featured',
        ]);
        ...
        $amount = $request->plan === 'paid_featured' ? 100 : 50;
    ...
    private function createStripeCheckoutSession(ContributorPayment $payment): ?string
    {
        $response = Http::withToken(config('services.stripe.secret'))
            ...
            ->post('https://api.stripe.com/v1/checkout/sessions', [
                'mode' => 'payment',
                'success_url' => url('contributor/payment/success') . '?session_id={CHECKOUT_SESSION_ID}',
                'cancel_url' => url('contributor/payment/cancel') . '?payment=' . $payment->id,
                'customer_email' => $payment->email,
                'metadata[payment_id]' => $payment->id,
                'payment_method_types[0]' => 'card',
                'line_items[0][price_data][currency]' => $payment->currency,
                'line_items[0][price_data][unit_amount]' => $payment->amount * 100,
                'line_items[0][price_data][product_data][name]' => $payment->plan === 'paid_featured'
                    ? 'Featured Contributor Account'
                    : 'Contributor Account',
                'line_items[0][price_data][product_data][description]' => $payment->plan === 'paid_featured'
                    ? 'One-time contributor account with featured contributor post placement.'
                    : 'One-time contributor account for guest blogging access.',
    ...
    private function finalizePaidContributor(ContributorPayment $payment, array $session): ContributorPayment
    {
        ...
        if (!$user) {
            ...
            $user->update([
                ...
                'designation' => $user->designation ?: $payment->designation,
                'intro' => $user->intro ?: $payment->intro,
                'reason_for_joining' => $user->reason_for_joining ?: $payment->reason_for_joining,
            ]);
        }
