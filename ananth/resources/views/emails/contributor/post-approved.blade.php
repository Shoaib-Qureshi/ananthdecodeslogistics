...
<p>Congratulations. Your work is now live on The Expert Desk.</p>
</div>
<div class="body">
<p>Hi <strong>{{ $post->author->name }}</strong>,</p>
<p>Great news! Your post has been reviewed and published on <strong>The Expert Desk</strong> at Ananth Decodes Logistics.</p>
<div class="post-box">
<strong>{{ $post->title }}</strong><br>
<span style="font-size:.85rem;color:#065f46;">Published {{ $post->published_at?->format('d M Y') }}</span>
</div>
<a href="{{ route('contributors.show', $post->slug) }}" class="btn">View Your Expert Desk Post</a>
<p style="margin-top:20px;">Share it with your network and keep contributing.</p>
</div>
<div class="footer">&copy; {{ date('Y') }} Ananth Decodes Logistics. This email was sent to {{ $post->author->email }}.</div>
</div></body></html>
