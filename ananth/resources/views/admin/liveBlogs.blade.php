                        ...
                        <tbody>
                            @if ($searchblog->count() > 0)
                                @foreach ($searchblog as $item)
                                    <tr>
                                        <th>{{ $item->id }}</th>
                                        <th><img width="150px" src="/media/{{ $item->thumbnail }}" alt=""></th>
                                        <td>
                                            <a class="link-primary" target="_blank"
                                                href="{{ asset('blog/' . $item->slug) }}/">{{ $item->title }}</a>
                                        ...
                                        <td>
                                            <form method="POST" action="{{ route('admin.blogs.delete', $item->id) }}" onsubmit="return confirm('Delete this blog post?');">
                                                @csrf
